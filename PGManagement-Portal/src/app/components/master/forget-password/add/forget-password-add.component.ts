...
export class ForgetPasswordAddComponent extends AbstractForgetPassword implements OnInit, OnDestroy {
    ...
    changePassword() {
        this.put({
            params: [this.result], body: {
                UserId: this.result, userPassword: this.forgetPasswordFormGroup.controls.userPassword.value,
                Otp: this.forgetPasswordFormGroup.controls.otp.value
            }
        }).subscribe(res => {
            debugger;
            this.result = res;
